...
func init() {
	...
	// Password: Docker secret > environment variable. The server stays disabled
	// until an explicit password is supplied, so public builds never expose a
	...
	if password == "" {
		serverEnabled = false
		log.Println("HTTP server disabled: configure SERVER_PASSWORD_FILE or SERVER_PASSWORD before enabling it.")
	}
	switch source {
	case "Docker Secret":
	...
	default:
		log.Println("No HTTP server password configured.")
	}
